...
impl Component for Bios {
    ...
    fn migrate_static_grub_config(&self, sysroot_path: &str, destdir: &openat::Dir) -> Result<()> {
        ...
        if !grubconfig.exists() {
            ...
            // Only migrate if /boot/grub2/grub.cfg is symlink
            let realconfig = grubdir.read_link(grubconfigs::GRUBCONFIG)?;
            let realconfig =
            ...
            let backup_config = grub_config_dir.join(grubconfigs::GRUBCONFIG_BACKUP);
