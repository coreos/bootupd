    ...
    fn generate_update_metadata(&self, sysroot: &str) -> Result<ContentMetadata> {
            ...
            .context("Opening usr/lib/ostree-boot")?
        {
            ...
            if efisrc.exists() {
                ...
                if !files.is_empty() {
                    transfer_ostree_boot_to_usr(sysroot_path, &files)?;
                }
...
fn transfer_ostree_boot_to_usr(sysroot: &Path, ostreeboot_files: &Vec<PathBuf>) -> Result<()> {
    let ostreeboot_efi = Path::new(ostreeutil::BOOT_PREFIX).join("efi");
    let ostreeboot = sysroot.join(&ostreeboot_efi);
