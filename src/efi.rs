...
impl Component for Efi {
    ...
    fn generate_update_metadata(&self, sysroot: &str) -> Result<ContentMetadata> {
        ...
        let meta = if efilib_path.exists() {
            ...
            let efi_components_iter = get_efi_component_from_usr(&sysroot_path, EFILIB)?;
            let mut efi_components: Vec<EFIComponent> = efi_components_iter.flatten().collect();
            efi_components.sort_by(|a, b| a.name.cmp(&b.name));
...
fn get_efi_component_from_usr<'a>(
    sysroot: &'a Utf8Path,
    usr_path: &'a str,
) -> Result<impl Iterator<Item = Option<EFIComponent>> + 'a> {
    ...
    Ok(WalkDir::new(&efilib_path)
        ...
        .map(move |entry| {
            let entry = match entry {
                Ok(e) if e.file_type().is_dir() && e.file_name() == "EFI" => e,
                _ => return None,
            };
...
Boot0003* test";
    ...
    fn test_get_efi_component_from_usr() -> Result<()> {
        ...
        let efi_comps_iter = get_efi_component_from_usr(utf8_tpath, EFILIB)?;
        let mut efi_comps: Vec<EFIComponent> = efi_comps_iter.flatten().collect();
        efi_comps.sort_by(|a, b| a.name.cmp(&b.name));
        assert_eq!(
            efi_comps,
            vec![
                EFIComponent {
        ...
        let efi_comps_iter = get_efi_component_from_usr(utf8_tpath, EFILIB)?;
        let efi_comps: Vec<Option<EFIComponent>> = efi_comps_iter.collect();
