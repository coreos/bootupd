...
impl Component for Efi {
    ...
    fn generate_update_metadata(&self, sysroot_path: &str) -> Result<ContentMetadata> {
        println!("==get sysroot_path = {:?}", sysroot_path);
        ...
        let meta = if efi_lib.exists() {
            ...
            println!("==get efi_paths = {:?}", efi_paths);

            // Scan all files under EFI path and remove the prefix sysroot
            let all_files: Result<Vec<String>> =
                efi_paths.iter().try_fold(Vec::new(), |mut acc, efi| {
                    ...
                    println!("==scan efi {:?}", efi);
                    let parent = if sysroot_path != "/" {
                        efi.strip_prefix(sysroot_path).unwrap_or(&efi)
                    } else {
                        efi
                    };
                    println!("==get parent {}", parent.display());

                    // Get all files and add parent path
                    let mut files = crate::util::filenames(&efidir)?
                        .into_iter()
                        .map(|mut f| {
                            f.insert_str(0, parent.to_str().unwrap());
                            f
                        })
                        .collect::<Vec<String>>();
            ...
            println!("==scan all_files {:?}", all_files);
